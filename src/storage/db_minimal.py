...
- 每次新交易更新 accounts.current_balance
"""
...
class MinimalDB:
    ...
    def __init__(self, db_path: str = "./data/finance_minimal.db"):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS accounts (
                    ...
                    current_balance DECIMAL(15,2) DEFAULT 0,  -- 当前余额（核心）
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS transactions (
                    ...
                    currency TEXT DEFAULT 'CNY',
                    balance_after DECIMAL(15,2),      -- 交易后余额（关键）
                    counterparty_name TEXT,           -- 商户
                    channel_name TEXT,                -- 支付渠道
                    raw_data TEXT,
                    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    def get_account_balance(self, account_id: str) -> Optional[Decimal]:
        ...
        with self._get_connection() as conn:
            ...
            row = cursor.fetchone()
            return Decimal(str(row['current_balance'])) if row else None
            ...
            try:
                ...
                cursor.execute('''
                    INSERT INTO transactions (
                        id, account_id, transaction_time, transaction_type,
                        amount, currency, balance_after, counterparty_name,
                        channel_name, raw_data
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    transaction_id,
                    account_id,
                    raw_transaction.transaction_time.isoformat(),
                    raw_transaction.transaction_type,
                    str(raw_transaction.amount),
                    raw_transaction.currency,
                    str(balance_after),
                    raw_transaction.counterparty.name if raw_transaction.counterparty else None,
                    raw_transaction.channel.name if raw_transaction.channel else None,
